...
- `backend/src/features/ships/refuel.test.ts` covers refuel transfer behavior, including idle/same-planet validation, tank capacity, source balance rollback, and concurrent transfer serialization so a refueler cannot be overdrawn.
...
- `0034_aspiring_tattoo.sql` adds fuel-capacity and Jump Fuel columns with defaults.
