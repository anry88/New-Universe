...
export function ShipsPage() {
  ...
  const activeTab = new URLSearchParams(location.search).get('tab') === 'shipyard' ? 'shipyard' : 'fleet';
  ...
  const shipyardPlanets = useMemo(() => {
    return planets.filter((planet) =>
      (planet.buildings ?? []).some((building) => building.typeId === 'shipyard'),
    );
  }, [planets]);

  const resolvedPlanetId =
    selectedPlanetId ??
    shipyardPlanets[0]?.id ??
    homePlanetId ??
    null;
  ...
  const canBuildShip = (planet: Planet, shipTypeId: string) => {
    ...
    return type.requiredBuildings.every((req) => hasRequiredBuilding(planet.buildings ?? [], req.typeId, req.level));
  };
  ...
  const handleBuildShip = async (typeSlug: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to queue ship build';
      alert(message);
    }
  };
      ...
      <div className="page-head" style={{ position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            ...
            <div className="page-title">{activeTab === 'shipyard' ? 'Shipyard' : 'Fleet Roster'}</div>
          </div>
        </div>
        <div className="page-stat">
          <div className="ps-v">{activeTab === 'shipyard' ? shipyardPlanets.length : ships.length}</div>
          <div className="ps-l">{activeTab === 'shipyard' ? 'SHIPYARDS' : 'VESSELS'}</div>
        </div>
      </div>

      <div style={{ position: 'relative', zIndex: 2, display: 'flex', gap: 8, padding: '0 12px 8px' }}>
        <button
          type="button"
              ...
              <>
                ...
                {(shipTypes ?? []).map((type) => {
                  const requirements = type.requiredBuildings
                    ...
                    .join(', ');
                  const canBuild = selectedPlanet ? canBuildShip(selectedPlanet, type.id) : false;

                  return (
                    <div key={type.id} className="ship-row">
                      <div className="ship-cls">
                        {SHIP_CLASS_TAG[type.id.toLowerCase()] ?? type.id.slice(0, 6).toUpperCase()}
                      </div>
                      <div>
                        <div className="ship-name">{type.name.en}</div>
                        <div className="ship-loc">
                          Build time: {Math.floor(type.buildTimeSec / 60)}m {type.buildTimeSec % 60}s
                        </div>
                        <div className="ship-loc">
                          Cost:{' '}
                          {Object.entries(type.buildCost)
                            .map(([resourceId, amount]) => `${getResourceSymbol(resourceId)} ${amount}`)
                            .join(' · ')}
                        </div>
                        <div className="ship-loc">Requires: {requirements || 'None'}</div>
                        <button
                          type="button"
                          disabled={!selectedPlanet || !canBuild || buildShip.isPending}
                          ...
                          style={{
                            ...
                            padding: '6px 12px',
                            fontSize: 11,
                            opacity: !selectedPlanet || !canBuild || buildShip.isPending ? 0.45 : 1,
                            cursor: !selectedPlanet || !canBuild || buildShip.isPending ? 'not-allowed' : 'pointer',
                          }}
                        >
                          {buildShip.isPending ? 'QUEUING...' : canBuild ? 'BUILD SHIP' : 'LOCKED'}
                        </button>
                      </div>
