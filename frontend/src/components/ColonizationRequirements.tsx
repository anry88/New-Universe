...
export const ColonizationRequirements: React.FC<ColonizationRequirementsProps> = ({
  ...
  const requirements: Requirement[] = [
    ...
    },
    {
      ...
      current: details?.cooldownRemainingSec ? `${Math.ceil(details.cooldownRemainingSec / 60)}m` : 'Ready',
      required: 'Ready',
      icon: Timer,
    },
  ];
  ...
  return (
    <div className="space-y-4 p-4 rounded-xl bg-slate-900/50 border border-slate-800">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="font-display font-semibold text-sm">FOUNDING REQUIREMENTS</h3>
      </div>
      ...
      <div className="pt-4 border-t border-slate-800">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-semibold text-slate-300">FOUNDING COSTS</span>
        </div>
      ...
      {!eligibility.allowed && (
        <div className="mt-4 p-3 rounded-lg bg-red-500/10 border border-red-500/20 flex gap-3 items-start">
          ...
          <p className="text-xs text-red-200 leading-relaxed">
            {eligibility.reason || 'Founding blocked by local constraints.'}
          </p>
        </div>
      )}
    </div>
