...
- `components/` — reusable presentational components.
  ...
  - **`ExpeditionDialog.tsx`** — mission launch on a **single** Galaxy map: `CosmicSystemRenderer` with `expeditionPick` (tap from the star to set a flat sector XY route point for scouts, or choose a discovered target planet for colonizers), safe-area layout, shared same-system planet-distance ETA/fuel preview for colonizers (round-trip except one-way deployment), and no manual fuel or ΔZ input.
