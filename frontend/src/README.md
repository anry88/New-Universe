...
- `pages/` — page-level components routed by `react-router-dom`.
  ...
  - **`PlanetDetail.tsx`** — detailed planet view with building slots, upgrade options, and production entry points for manual processors; derives construction eligibility from `GET /buildings/types` limits plus live `/me` planets/research/deposit richness via `@shared/types/building-eligibility`, ignores buildings still in the initial queue for dependency previews, and keeps merely discovered planets read-only until `/me` marks them `isColonized`.
...
- `components/` — reusable presentational components.
  ...
  - **`ProductionDialog.tsx`** — production-order sheet for buildings with recipes. It fetches `/resources/production/recipes`, previews selected quantity via `/resources/production/preview`, confirms input spend, starts orders through `/resources/production/start`, and lists queued orders.
