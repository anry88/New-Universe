...
- `components/` — reusable presentational components.
  ...
  - **`ProductionDialog.tsx`** — compact production-order sheet for buildings with recipes. It fetches `/resources/production/recipes`, previews selected output quantity via `/resources/production/preview`, shows required inputs, duration, and queued-order remaining-time countdowns, confirms input spend, and starts orders through `/resources/production/start`.
