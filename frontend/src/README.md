...
- `components/` — reusable presentational components.
  ...
  - **`ExpeditionDialog.tsx`** — mission launch on a **single** Galaxy map: `CosmicSystemRenderer` with `expeditionPick` for local home-system routes, a Jump Gate route selector for known public destinations and their safe planet summaries, shared ETA/fuel preview for local and gate routes, stored Jump Fuel blocking, one-way colonizer deployment, and no manual fuel or ΔZ input.
