...
- `components/` — reusable presentational components.
  ...
  - **`cosmic/SystemMap.tsx`** — exports `CosmicSystemRenderer` (SVG orbital home-system map; pan/pinch/zoom) and optional `expeditionPick` for mission UI: tap-to-aim sector ΔXY from the star, draft jump trail from `launchPlanetId`, planets/colony UI suppressed while picking.
  ...
  - **`ExpeditionDialog.tsx`** — mission launch on a **single** Galaxy map: `CosmicSystemRenderer` with `expeditionPick` (tap from the star to set sector XY, ΔZ field, draft trail from the launch planet), safe-area layout, auto round-trip fuel (optional manual override).
