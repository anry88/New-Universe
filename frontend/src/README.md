...
- `lib/` — shared infrastructure (API client, store, Sentry init, helpers).
  ...
  - **`ship-build-eligibility.ts`** — `resolveShipBuildBlockedReason(planet, shipType, researchRows?)` for local shipyard planner blocking on required building level, mirrored ship research gates, and available build-cost resources.
...
- `components/` — reusable presentational components.
  ...
  - **`cosmic/resources.ts`** — resource id -> symbol/label dictionary used in cosmic UI; aligned with seeded resource ids (`iron`, `silicon`, `oil`, `tritium`, `jump_fuel`, etc.) for tech-tree cost rendering.
  - **`cosmic/ships.tsx`** — shared Cosmic Atlas ship icon resolver used by the orbital map, fleet roster, shipyard planner, expedition dialog, and cargo dialog. It defines unique hull icons for active ship catalog ids, including `recon_probe` as the one-use Recon Probe for random Jump Gate discovery.
...
- **`ship-build-eligibility.ts`** — local planner helper that returns the first ship build blocker (`missingBuilding`, `missingResearch`, or `insufficientResource`) for localized shipyard rows.
