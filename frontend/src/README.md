...
- `lib/` — shared infrastructure (API client, store, Sentry init, helpers).
  ...
  - **`expedition-routing.ts`** — `buildExpeditionPreview()` wraps shared expedition route math for local and Jump Gate mission previews, including one-way colonizer fuel, ETA, and stored Jump Fuel requirements.
  ...
  - **`ship-build-eligibility.ts`** — `resolveShipBuildBlockedReason(planet, shipType, researchRows?)` for local shipyard planner blocking on required building level, mirrored ship research gates, and available build-cost resources; UI formatting uses localized entity labels instead of raw ids.
  - **`ship-build-eligibility.test.ts`** — unit coverage for cargo_light shipyard L2, Logistics L1, and capital-resource build blockers.
...
- `hooks/` — custom React hooks.
  ...
  - **`useShips.ts`** — `useBuildShip()` wraps `POST /ships/build` with the same optimistic queue pattern as research: writes a temporary building ship into `['me']` plus `['ship-queue']`, rolls back on error, replaces temp ids from the server queue metadata on success, and invalidates both caches on settle. `useShipQueue()` still handles due-time refresh and `useRushShip()` refreshes both caches after rush.
...
- `pages/` — page-level components routed by `react-router-dom`.
  ...
  - **`SystemMap.tsx`** — page component for the interactive home system map; links into the sector radar for the same sector cube, treats only `isColonized` planets as openable settlements, and exposes the outer-orbit Jump Gate panel with locked/unlocked state, Random jump launch, known-destination badges, and route action deep-links.
  ...
  - **`Ships.tsx`** — fleet management screen with two tabs: roster (`/ships`) and shipyard build queue/planner (`/ships?tab=shipyard`) wired to optimistic `POST /ships/build`; rows use shared Cosmic ship icons, show localized blockers for missing building levels and build-cost resources, warn before colonizer construction when the current `/me.colonization` cap is already reached, ships in `building` never fall back to idle while `/ships/queue` is loading, show ETA/progress from merged `/me` + `/ships/queue` state, expose rush completion through the fixed bottom `QueueStrip` via `POST /ships/rush`, route logistics ships into the Colonies cargo-transfer dialog with `cargoShip` preselection, route Recon Probes to the Jump Gate panel, and honors `route=jump_gate&destinationSystemId=<id>` deep-links from the Jump Gate destination panel.
...
- `components/` — reusable presentational components.
  ...
  - **`cosmic/resources.ts`** — resource id -> symbol/label dictionary used in cosmic UI; aligned with seeded resource ids (`iron`, `silicon`, `oil`, `tritium`, `antimatter`, `jump_fuel`, etc.) for tech-tree cost rendering and falls back through `@shared/types/entity-labels` so unknown resources humanize rather than showing raw slugs.
  ...
  - **`cosmic/ships.tsx`** — shared Cosmic Atlas ship icon resolver used by the orbital map, fleet roster, shipyard planner, expedition dialog, and cargo dialog. It defines unique hull icons for active ship catalog ids, including `recon_probe` as the one-use Recon Probe for random Jump Gate discovery, and placeholder labels use localized/humanized text instead of raw ids.
  - **`cosmic/ships.test.ts`** — Vitest coverage for active ship icon resolution, visible `recon_probe` lists, lifecycle tones, and unknown-hull fallback behavior.
  ...
  - **`cosmic/SystemMap.tsx`** — exports `CosmicSystemRenderer` (SVG orbital home-system map; pan/pinch/zoom) and optional `expeditionPick` for mission UI: tap-to-aim a flat sector ΔXY route point, colonizer target selection on discovered planets, draft trail from `launchPlanetId`, active expedition trails filtered to in-flight/returning rows, eight one-planet orbit guide rings that ignore obfuscated `unknown` placeholders from `/me`, a clickable outer-orbit Jump Gate marker, shared Cosmic ship icons parked near their planets or interpolated along routes, and undiscovered planets hidden from their real map positions until the backend marks them discovered. Static map layers and CSS route lines are memoized so route-heavy maps and expedition aim drags avoid repainting every orbit/planet each movement tick. The selected-planet card now also shows mineable deposits and can be dismissed by tapping free space.
  ...
  - **`UpgradeDialog.tsx`** — Cosmic Atlas bottom sheet for completed buildings. It shows shared L10 / Command Center cap blockers, level-up costs from `@shared/config/buildingUpgradeEconomy` including L6+ extra materials, resource-specific extractor yield previews, shipyard/production entry points, saved extractor target, energy-anomaly operational-demand suppression, and `selectedResourceId` switching through `POST /buildings/resource` with the same deposit-limit chips and localized block reasons used during construction.
  ...
  - **`ExpeditionDialog.tsx`** — mission launch on a **single** Galaxy map: `CosmicSystemRenderer` with `expeditionPick` for local home-system routes, a Jump Gate route selector for known public destinations and their safe planet summaries, shared ETA/fuel preview for local and gate routes, stored Jump Fuel blocking, one-way colonizer deployment with `/colonies/eligibility` preflight warnings for cooldown and colony-cap blockers, localized launch errors, and no manual fuel or ΔZ input.
