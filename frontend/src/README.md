...
- `lib/` — shared infrastructure (API client, store, Sentry init, helpers).
  ...
  - **`fleet.ts`** — `isCargoTransferShipType`, `isCargoTransferShip`, and `formatCargoTransferError` keep cargo-transfer entrypoints limited to logistics ships and map server cargo / fuel / Jump Fuel failures to localized UI copy.
...
- **`fleet.ts`** — cargo/fleet helper module. `isCargoTransferShipType()` and `isCargoTransferShip()` keep only logistics ships in cargo-transfer flows, while `formatCargoTransferError()` converts known server cargo, fuel, Jump Fuel, and known-common-system errors into localized dictionary keys.
