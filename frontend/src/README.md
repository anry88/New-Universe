...
- `lib/` — shared infrastructure (API client, store, Sentry init, helpers).
  ...
  - **`fleet.ts`** — `isCargoTransferShipType`, `isCargoTransferShip`, and `formatCargoTransferError` keep cargo-transfer entrypoints limited to logistics ships and map server cargo / Jump Fuel failures to localized UI copy.
...
- `components/` — reusable presentational components.
  ...
  - **`CargoTransferDialog.tsx`** — interplanetary logistics interface for moving resources between colonies through the shared `CargoTransferRequest` contract; inter-system targets can opt into `routeMode='jump_gate'`, then show and block on the stored Jump Fuel cost at the origin planet.
...
- **`fleet.ts`** — cargo/fleet helper module. `isCargoTransferShipType()` and `isCargoTransferShip()` keep only logistics ships in cargo-transfer flows, while `formatCargoTransferError()` converts known server cargo and Jump Fuel errors into localized dictionary keys.
