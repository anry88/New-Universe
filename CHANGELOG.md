...
- Селектор «Сектор» на системной карте теперь заблокирован до открытия первой дополнительной системы через Jump Gate; после random jump он показывает список известных систем, а выбранная система открывается как закрытая карта со звездой и порталом. Jump Gate scout-маршруты теперь выбирают точку в открытой системе и считают топливо по маршруту планета старта -> домашний портал -> портал назначения -> выбранная точка, плюс отдельный расход Jump Fuel.
