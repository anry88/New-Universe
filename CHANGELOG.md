...
### Removed
- `issue #392`: убраны устаревшие военные корпуса `fighter`/`cruiser`/`battleship`, дублирующие лёгкую/среднюю/тяжёлую линии истребителей, бомбардировщиков и лазерных кораблей. Удалены из seed-каталога, shared entity-labels, ship research gates и Cosmic Atlas резолвера; добавлен `HIDDEN_SHIP_TYPE_IDS` и data migration `0037_cleanup_duplicate_warships`, которая удаляет осиротевшие строки `ship_types` (и каскадом — `ships`/`expeditions`) в существующих dev-БД, чтобы единственным источником истины военного дерева стала weight-class-линия.
