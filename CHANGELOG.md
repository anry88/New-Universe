...
### Added
- `P2.2-018` (issue #329): добавлен общий backend helper посадочных слотов `spaceport`, учитывающий докованные/строящиеся корабли и активные рейсы к целевой планете.
...
### Changed
- `P2.2-018` (issue #329): обычные целевые посадки и строительство кораблей теперь требуют свободный слот `spaceport`; разведка и колонизатор остаются исключениями, а колонизатор разворачивает Command Center без посадочного слота.
