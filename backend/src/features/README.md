...
- **`jump.ts`** — `jumpShip(userId, request, options?)` handles specialized Jump Ship teleportation. Checks for unlocked Jump Gate state, deducts 50 stored `jump_fuel` from the current planet, chooses server-authoritative random sectors or known `destinationSystemId` targets, lazily generates common-pool sectors, excludes protected Home Systems, and upserts system-level known destination records without revealing all planets.
