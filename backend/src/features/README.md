...
- **`visibility.ts`** — exports `checkVisibility(shipId, tx?)` which resolves the ship's current position and sensor range, then finds all systems (and their planets) within Euclidean distance of `sensorRange` that are not yet discovered by the ship's owner. Skips foreign home systems. Batch-inserts new `discovered_systems` and `discovered_planets` rows and returns the array of new `Discovery` objects. Used by the expedition tick worker (`tick_expeditions`).
- **`visibility.test.ts`** — Vitest coverage asserting: planet discovered within range, planet outside range not discovered, no duplicate discoveries, foreign home system invisible, own home system visible, all planets in a newly visible system discovered, system+planet discovery, missing ship returns empty, and 3D Euclidean distance respects sensor range.
