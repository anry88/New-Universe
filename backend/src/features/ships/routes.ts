...
export async function shipsRoutes(app: FastifyInstance) {
    ...
    async (request, reply) => {
      ...
      if (!planetId || !typeSlug) {
        ...
        return reply.status(400).send({
          ...
          message:
            locale === "ru"
              ? "Выберите планету и тип корабля."
              : "Select a planet and ship type.",
        });
      }
      ...
      if (!result.success) {
        ...
        const details = result.code
          ? ({
              code: result.code,
              ...(result.details ?? {}),
            } as ShipBuildErrorDetails)
          : null;
        return reply.status(result.status).send({
          error: details
            ? formatShipBuildErrorMessage(details, locale)
            : result.error,
          code: result.code,
      ...
      if (!result.success) {
        ...
        const details = result.code
          ? ({
              code: result.code,
              ...(result.details ?? {}),
            } as RefuelErrorDetails)
          : null;
        return reply.status(result.status).send({
          error: details
            ? formatRefuelErrorMessage(details, locale)
            : result.error,
          code: result.code,
          details: result.details,
        });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const req = request.body as RefuelReplenishRequest;
      const result = await replenishRefueler(auth.userId, req);
