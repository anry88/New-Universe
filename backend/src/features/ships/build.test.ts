...
describe("Ship Building - POST /ships/build", () => {
  ...
  it("blocks cargo_light until shipyard L2 and Logistics L1, then builds from capital resources", async () => {
    ...
    await db
      ...
      .set({ level: 2 })
      .where(
        and(eq(buildings.planetId, planetId), eq(buildings.typeId, "shipyard")),
      );
    ...
    expect(researchBlockedResponse.json().error).toContain(
      "Logistics research level 1",
    );
  ...
  it("light_fighter: blocks without military_shipyard, then builds with Weapons I", async () => {
    ...
    await db
      .insert(buildings)
      .values({ planetId, typeId: "shipyard", slotIndex: 0, level: 2 });
    await ensureSpaceport(planetId);
    await ensureResource(planetId, "iron", 500);
    ...
    await db
      ...
      .values({ userId, branch: "weapons", level: 1 });
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(buildings.typeId, "military_shipyard"),
        ),
      );
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(researchProgress.branch, "weapons"),
        ),
      );
  ...
  it("light_laser: requires Military Shipyard L2, Weapons II, and liquid_nitrogen in build cost", async () => {
    ...
    await db
      .insert(researchProgress)
      .values({ userId, branch: "weapons", level: 2 });
  ...
  it("ship catalog contains light, medium, heavy and rocket combat hulls with correct stats", async () => {
    ...
    expect(lightFighter!.jumpFuelCapacity).toBe(250);
    expect(lightFighter!.refuelFuelCapacity).toBe(0);
    ...
    expect(lightBomber!.jumpFuelCapacity).toBe(400);
    ...
    expect(lightLaser!.jumpFuelCapacity).toBe(300);
    ...
    expect(mediumFighter!.requiredBuildings).toEqual([
    ...
    expect(heavyFighter!.requiredBuildings).toEqual([
    ...
    const heavyRocketCarrier = types.find(
      (t) => t.id === "heavy_rocket_carrier",
    );
    ...
    expect(smallShield!.combatStats.shields).toMatchObject({
      capacity: 700,
      radius: 1.5,
    });
    expect(mediumShield!.combatStats.shields).toMatchObject({
      capacity: 1800,
      radius: 3.5,
    });
    expect(largeShield!.combatStats.shields).toMatchObject({
      capacity: 4200,
      radius: 6,
    });
    expect(largeShield!.requiredBuildings).toEqual([
      { typeId: "military_shipyard", level: 6 },
    ]);
  });
  ...
  it("syncs only the current user ready ships and suppresses stale pending notifications", async () => {
    ...
    const updated = await db.query.ships.findFirst({
      where: eq(ships.id, readyShip.id),
    });
    expect(updated?.status).toBe("idle");
    expect(updated?.queueCompletesAt).toBeNull();

    const untouched = await db.query.ships.findFirst({
      where: eq(ships.id, otherReadyShip.id),
    });
