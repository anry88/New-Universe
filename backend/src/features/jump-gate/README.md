...
- **`service.ts`** — exports `getJumpGateState(userId)`. It derives unlock from completed `jump_drive >= 1`, creates the player's private `jump_gates` persistence row when unlocked, resolves the outer-orbit home anchor, finalizes due calibration rows, reports random-jump availability, and lists discovered public destination systems only, including their deterministic `seed`, registry `source` / `lastVisitedAt`, plus safe per-body summaries that distinguish unknown, discovered, occupied, and owned-colony planets without exposing private Home Systems.
