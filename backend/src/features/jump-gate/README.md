...
- **`service.ts`** — exports `getJumpGateState(userId)`. It derives unlock from completed `jump_drive >= 1`, creates the player's private `jump_gates` persistence row when unlocked, resolves the outer-orbit home anchor, finalizes due calibration rows, reports random-jump availability, and lists discovered public destination systems only, including their deterministic `seed`, neutral short-tag title data, total planet count, registry `source` / `lastVisitedAt`, plus safe per-body summaries that distinguish unknown, discovered, occupied, and owned-colony planets without exposing private Home Systems. Discovered public planets include mineable resource/richness rows for the map detail card.
- **`service.test.ts`** — Vitest coverage for locked state, active-but-unfinished Jump Drive research, unlocked persistence creation, public known destination filtering, discovered-planet resource projection, and due calibration finalization.
