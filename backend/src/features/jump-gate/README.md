...
- **`routes.ts`** — `jumpGateRoutes(app)` registers `GET /jump-gate/state`. Requires JWT auth and returns the shared `JumpGateStateResponse` for the current player.
- **`service.ts`** — exports `getJumpGateState(userId)`. It derives unlock from completed `jump_drive >= 1`, creates the player's private `jump_gates` persistence row when unlocked, resolves the outer-orbit home anchor, finalizes due calibration rows, reports random-jump availability, and lists discovered public destination systems only.
