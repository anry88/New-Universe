...
- **`service.ts`** — `syncTutorialProgress(userId)` inspects user actions (mine/storage/scout/expedition), updates `users.tutorial_step`, and grants a one-time completion reward (`+200 iron`, `+100 water`) to the home planet when step 5 is reached.
