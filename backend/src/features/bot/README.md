...
- **`commands.ts`** — `handleStartCommand(chatId)` sends the welcome message with an inline button to launch the Mini App using `env.PUBLIC_FRONTEND_URL`; `handleAddDiamondCommand(chatId, args, actor, options)` is the admin `/add_diamond` handler (with aliases), validates `env.ADMIN_TELEGRAM_IDS`, updates the target user’s wallet via `features/resources/wallet.ts`, logs the action, and returns localized success/error text.
- **`webhook.ts`** — `handleTelegramUpdate(update)` is the dispatcher that parses incoming Telegram updates and routes them to the appropriate command handlers (`/start`, `/add_diamond` aliases).
