...
- **`jump.ts`** — `jumpShip(userId, request, options?)` handles Jump Gate inter-sector jumps:
  ...
  - For random discovery, requires a `recon_probe`, chooses deterministic server-side candidate sectors, lazily generates common-pool systems, and consumes the probe after opening the public neutral system; private Home Systems are never valid jump targets.
