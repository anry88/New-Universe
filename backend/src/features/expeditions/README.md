...
- **`routes.ts`** — `expeditionsRoutes(app)` registers:
  - `POST /` — launches a standard expedition to a route point. Accepts `{ shipId, targetX, targetY, targetZ, cargoLoaded }`; fuel is calculated and reserved server-side from route distance and ship fuel consumption. `targetPlanetId` is valid only for recon surveys and colonizer deployments. Colonizers must provide a discovered, eligible target planet and reserve one-way fuel because a successful arrival consumes the ship into the new Command Center.
...
- **`launch.ts`** — `launchExpedition(userId, request)` performs the launch flow:
  ...
  - Calculates fuel from **sector XY** route distance (galactic plane; Z is ignored) × `ship_types.fuel_consumption`, then spends that amount from the launch planet via `spendResources`. Normal expeditions reserve round-trip fuel; colonizer deployments to `targetPlanetId` reserve one-way fuel.
