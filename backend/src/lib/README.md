...
- **`metrics.ts`** — Prometheus text exporter for task `P4-OPS-001`. Exports `collectNewUniverseMetrics()`, `recordHttpRequest()`, `recordPlayerActivity()`, and `formatMetricSamples()`. `/metrics` includes API uptime, HTTP counters/histograms, Postgres/Redis health, source-of-truth game queue backlog/due age, and aggregate product analytics from `player_activity_daily` for DAU/WAU/MAU, observed play time, sessions, and system-development averages.
- **`metrics.test.ts`** — verifies Prometheus text formatting, label escaping, and histogram family metadata.
