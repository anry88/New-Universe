...
- **`metrics.ts`** — Prometheus text exporter for task `P4-OPS-001`. Exports `collectNewUniverseMetrics()`, `recordHttpRequest()`, `startPlayerActivitySession()`, `recordPlayerActivity()`, `formatActivityTimestamp()`, and `formatMetricSamples()`. `/metrics` includes API uptime, HTTP counters/histograms, Postgres/Redis health, source-of-truth game queue backlog/due age, and cached aggregate analytics from `player_activity_daily` plus gameplay/monetization tables for DAU/WAU/MAU, absolute player deltas, observed play time, sessions, system-development averages, progression milestones, and Telegram Stars checkout funnel metrics. Online sessions start only through `POST /me/session/start`; later visible frontend requests carrying `X-NU-Online-Activity: 1` extend play time from the previous online request without inactivity-gap splitting or a heartbeat cap. The heavier product/progression/monetization aggregates are cached for 5 minutes so routine VictoriaMetrics scrapes stay light.
