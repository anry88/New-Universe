...
- **`metrics.ts`** — Prometheus text exporter for task `P4-OPS-001`. Exports `collectNewUniverseMetrics()`, `recordHttpRequest()`, `startPlayerActivitySession()`, `recordPlayerActivity()`, `formatActivityTimestamp()`, `formatMetricSamples()`, and `metricsMachineId()`. `/metrics` includes API uptime, machine-scoped HTTP counters/histograms, Postgres/Redis health, source-of-truth game queue backlog/due age, and cached aggregate analytics from `player_activity_daily` plus gameplay/monetization tables for DAU/WAU/MAU, absolute player deltas, registration-source breakdowns, observed play time, sessions, system-development averages, progression milestones, and Telegram Stars checkout funnel metrics. Online sessions start only through `POST /me/session/start`; later visible frontend requests carrying `X-NU-Online-Activity: 1` extend play time from the previous online request without inactivity-gap splitting or a heartbeat cap. The heavier product/progression/monetization aggregates are cached for 5 minutes so routine VictoriaMetrics scrapes stay light.
- **`metrics.test.ts`** — verifies Prometheus text formatting, label escaping, histogram family metadata, signed product deltas, and progression milestone labels.
...
- **`telegram.ts`** — Telegram Mini App `initData` validation per the [official algorithm](https://core.telegram.org/bots/webapps#validating-data-received-via-the-mini-app):
  ...
  - `TelegramInitData` interface (`user`, `chat_instance`, `chat_type`, optional `start_param`, `auth_date`, `hash`).
  ...
  - `callTelegramBotApi`, `callTelegramBotApiDetailed`, `sendTelegramMessage`, `sendTelegramMessageDetailed`, `isTelegramBotBlockedByUser`, `answerPreCheckoutQuery`, `createTelegramInvoiceLink`, `getStarTransactions`, and `refundStarPayment` wrap Bot API calls used by notifications, `/start`, Telegram Stars invoices, Stars pre-checkout, missed-payment reconciliation, and refund support. The detailed variants preserve status/error metadata so the notification worker can treat blocked-bot responses as terminal delivery failures.
