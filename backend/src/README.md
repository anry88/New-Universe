...
- [`features/`](features/README.md) — feature modules (`auth`, `bot`, `me`, `world`). Each feature owns its own `service.ts`, `routes.ts`, and tests.
...
- **`index.ts`** — the application entry point. Order of operations:
  ...
  5. Registers `healthRoutes`, `botRoutes`, and `authRoutes` (the latter mounted at the `/auth` prefix).
