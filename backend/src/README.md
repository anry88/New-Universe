...
- [`features/`](features/README.md) — feature modules (`auth`, `bot`, `me`, `world`, `buildings`, `resources`, `ships`, `expeditions`, `research`, `tutorial`, `market`, `multiplayer`). Each feature owns its own route handlers, service logic, and tests.
...
- **`index.ts`** — the application entry point. Order of operations:
  ...
  5. Registers `healthRoutes`, `botRoutes`, `authRoutes` (mounted at `/auth`), `meRoutes` (mounted at `/me`, including active-session timer finalization), `buildingsRoutes` (mounted at `/buildings`), `resourcesRoutes` (mounted at `/resources`, including `/resources/production/*`), `shipsRoutes` (mounted at `/ships`), `expeditionsRoutes` (mounted at `/expeditions`), `cargoRoutes` (mounted at `/cargo`, including one-way multi-load `/cargo/transfer`), `researchRoutes` (mounted at `/research`, including `/research/rush`), `tutorialRoutes` (mounted at `/tutorial`), and `marketRoutes` (`/market/offers`, `/market/orders`, `/market/orders/:orderId/cancel`).
