...
- [`features/`](features/README.md) — feature modules (`auth`, `bot`, `me`, `world`, `buildings`, `resources`, `ships`, `expeditions`, `research`). Each feature owns its own route handlers, service logic, and tests.
...
- **`index.ts`** — the application entry point. Order of operations:
  ...
  5. Registers `healthRoutes`, `botRoutes`, `authRoutes` (mounted at `/auth`), `meRoutes` (mounted at `/me`), `buildingsRoutes` (mounted at `/buildings`), `resourcesRoutes` (mounted at `/resources`), `shipsRoutes` (mounted at `/ships`), `expeditionsRoutes` (mounted at `/expeditions`), and `researchRoutes` (mounted at `/research`).
