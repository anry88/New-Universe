...
- **`buildings.ts`** — `building_types` (catalog) and `buildings` (instances). `building_types` carries `category`, optional nullable **`max_per_planet`** / **`max_global`** enforcement knobs (seeded via `catalog-rows.ts`), `maxLevel`, `deps` (JSONB `{ typeId, level }[]`), `baseCost` (JSONB `{ resourceId: amount }`), `baseTimeSec`, `baseOutput`, `energyConsumption`. `buildings` is per-planet with `level`, optional `queueAction` and `queueCompletesAt` columns used by the build queue.
