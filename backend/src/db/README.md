...
- **`production.ts`** — `production_orders` table. Stores explicit manufacturing jobs with `userId`, `planetId`, `buildingId`, `recipeId`, `quantity`, `status`, JSONB `inputs`/`outputs`, `startedAt`, `completesAt`, and `completedAt`; indexed by due status plus user/planet status for worker ticks and UI order lists.
...
- **`resources.ts`** — inserts `RESOURCE_CATALOG_ROWS` into `resources` (24 rows including `oil`, `fuel`, `steel`, `electronics`, and base/mineral tiers `water` … `biomass`).
...
Non-currency materials should be obtainable without the NPC broker: **planetary richness** (`richness`, exposed through `/me` as a deposit level), passive extraction buildings (`building_types.baseOutput` with `resourceId` + `baseRate`, applied on build/upgrade completion in `features/buildings/service.ts`), and explicit production orders (`features/resources/production.ts`). Starter home-system rows and newly colonized planets create inventory/deposit rows with `planet_resources.regenRate = 0`; passive extraction starts only after matching extractor buildings complete. Mines require local metal deposits and distribute their rate across local metal ids, drills require fluid/gas/ice deposits, **`oil`** is produced by **`oil_pump`**, and rare **`biomass`** by the slower **`biomass_harvester`** on planets with biomass deposits. **`smelter`**, **`refinery`**, **`fabrication_bay`**, and **`cryo_factory`** do not auto-generate resources; players choose recipes/quantities and pay inputs up front through `production_orders`. The market remains an optional liquidity sink, not the sole source of baseline alloys/components.
