CREATE TYPE "public"."market_offer_status" AS ENUM('active', 'paused', 'exhausted', 'archived');--> statement-breakpoint
CREATE TYPE "public"."market_order_status" AS ENUM('open', 'partially_filled', 'filled', 'cancelled', 'expired', 'failed', 'settled');--> statement-breakpoint
CREATE TYPE "public"."market_order_type" AS ENUM('market', 'limit');--> statement-breakpoint
CREATE TYPE "public"."market_scope" AS ENUM('npc', 'player');--> statement-breakpoint
CREATE TYPE "public"."market_side" AS ENUM('buy', 'sell');--> statement-breakpoint
...
ALTER TABLE "systems" ADD COLUMN "z" numeric(10, 2) NOT NULL;--> statement-breakpoint
DO $$ BEGIN
...
END $$;
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_offers_resource_side_status_idx" ON "market_offers" USING btree ("resource_id","side","status","created_at");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_offers_scope_status_created_idx" ON "market_offers" USING btree ("scope","status","created_at");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_order_fills_order_executed_idx" ON "market_order_fills" USING btree ("order_id","executed_at");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_order_fills_resource_executed_idx" ON "market_order_fills" USING btree ("resource_id","executed_at");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_orders_user_created_idx" ON "market_orders" USING btree ("user_id","created_at");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_orders_resource_state_created_idx" ON "market_orders" USING btree ("resource_id","status","created_at");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "market_orders_status_created_idx" ON "market_orders" USING btree ("status","created_at");