...
export const shipTypes = pgTable("ship_types", {
  ...
  jumpFuelCapacity: integer("jump_fuel_capacity").notNull().default(150),
  refuelFuelCapacity: integer("refuel_fuel_capacity").notNull().default(0),
  refuelJumpFuelCapacity: integer("refuel_jump_fuel_capacity")
    .notNull()
    .default(0),
  buildTimeSec: integer("build_time_sec").notNull(),
  buildCost: jsonb("build_cost").$type<Record<string, number>>().notNull(),
  requiredBuildings: jsonb("required_buildings")
    .$type<{ typeId: string; level: number }[]>()
    .notNull()
    .default([]),
  sensorRange: integer("sensor_range").notNull(),
  combatStats: jsonb("combat_stats")
    .$type<CombatStats>()
    .notNull()
...
export const ships = pgTable("ships", {
  ...
  cargoJson: jsonb("cargo_json")
    .$type<Record<string, number>>()
    .notNull()
    .default({}),
  fuel: numeric("fuel", { precision: 12, scale: 2 }).notNull().default("0"),
  jumpFuel: numeric("jump_fuel", { precision: 12, scale: 2 })
    .notNull()
    .default("0"),
  refuelFuel: numeric("refuel_fuel", { precision: 12, scale: 2 })
    .notNull()
    .default("0"),
  refuelJumpFuel: numeric("refuel_jump_fuel", { precision: 12, scale: 2 })
    .notNull()
    .default("0"),
