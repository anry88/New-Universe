...
- **`research-catalog.ts`** — Phase 2 research catalog: runtime-compatible backend wrapper around **`shared/config/researchCatalog.ts`** (seven branches × **five** tiers). Localized names/descriptions, costs/time scaling, typed effect modifiers, flattened `RESEARCH_TECH_TREE` consumed by research routes/effects/seeding.
