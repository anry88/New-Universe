...
- **`research-unlocks.ts`** — maps gated actions (specific buildings, advanced energy generators, advanced hulls, colonization, cargo transfers, NPC market orders, jump drive) to minimum completed `{ branch, level }` pairs; enforced by `features/research/gates.ts`. The lightweight transporter hull itself is unlocked by shipyard L2 through the ship catalog, while cargo-transfer usage still requires Logistics research.
