...
describe('Tick Expeditions Worker', () => {
  ...
  it('should interpolate position correctly in_flight', () => {
    ...
    const posStart = calculateExpeditionPosition(expedition, originSystem, new Date(400000));
    expect(posStart.x).toBe(0);

    const posMid = calculateExpeditionPosition(expedition, originSystem, new Date(700000));
    expect(posMid.x).toBe(50);

    const posEnd = calculateExpeditionPosition(expedition, originSystem, new Date(1000000));
  ...
  it('should discover new systems/planets during travel', async () => {
    ...
    const [midSystem] = await db.insert(systems).values({
      name: 'Mid System',
      sectorX: 50,
      sectorY: 0,
      sectorZ: 0,
    ...
    const [midPlanet] = await db.insert(planets).values({
