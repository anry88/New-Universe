...
- **`index.ts`** — worker entry point. It registers all workers (`createBuildingsWorker`, `createShipsWorker`, `createExpeditionsWorker`, `createNotificationsWorker`, `createCargoRoutesWorker`, `createResearchWorker`, `createProductionOrdersWorker`), manages their Redis connections, and handles graceful shutdown.
- **`cargo-routes.ts`** — handles `arrive_cargo` jobs in the `expeditions` queue. Completes one-way cargo transfers by applying aggregated resources to the target planet, moving the ship to its new location, and reusing the same completion helper as online expedition sync.
