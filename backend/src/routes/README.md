...
- **`cargo.ts`** — `cargoRoutes(fastify)` registers `POST /cargo/transfer`. Requires JWT auth. Delegates to `launchCargoTransfer` to send one transfer order with multiple resource load lines between two player-owned planets via a cargo ship.
