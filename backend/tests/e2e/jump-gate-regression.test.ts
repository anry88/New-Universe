...
async function setPlanetResource(planetId: string, resourceId: string, amount: number): Promise<void> {
  await db
...
async function setResearchLevel(userId: string, branch: string, level: number): Promise<void> {
  await db
...
async function rowsForUserPlanet(
  ...
  return db.query.discoveredPlanets.findMany({
    where: and(eq(discoveredPlanets.userId, userId), eq(discoveredPlanets.planetId, planetId)),
  });
}
...
describe('Jump Gate end-to-end regression suite', () => {
  ...
  it('full path: unlock -> random jump -> known destination -> discovery/colonization -> cargo delivery', async () => {
    ...
    const { token, user } = loginRes.json() as { token: string; user: { id: string } };
    ...
    const randomJump = randomJumpRes.json() as {
      ...
      destination: { systemId: string; source: string; lastVisitedAt: string | null };
    ...
    expect(knownJump.targetSystem.id, `${AREA.jump} repeat destination system`).toBe(randomJump.destination.systemId);
    expect(knownJump.destination.lastVisitedAt, `${AREA.jump} repeat visit timestamp`).not.toBeNull();

    const targetSystem = await db.query.systems.findFirst({
      where: eq(systems.id, randomJump.destination.systemId),
    });
    expect(targetSystem?.isHome, `${AREA.visibility} target is not home`).toBe(false);
    expect(targetSystem?.ownerId, `${AREA.visibility} target is public`).toBeNull();
    await db
      .update(systems)
    ...
    const targetPlanet = targetPlanets.sort((a, b) => a.name.localeCompare(b.name))[0];
    expect(targetPlanet, `${AREA.discovery} target common planet`).toBeDefined();
    if (!targetPlanet) {
    ...
    expect(stateRes.statusCode, `${AREA.visibility} GET /jump-gate/state`).toBe(200);
    const gateState = stateRes.json() as { knownDestinations: { systemId: string }[] };
    expect(
      gateState.knownDestinations.some((destination) => destination.systemId === randomJump.destination.systemId),
      `${AREA.visibility} random public destination remains known`,
    ).toBe(true);
    expect(
      gateState.knownDestinations.some((destination) => destination.systemId === foreignHomeSystem.id),
    ...
    expect(sectorAnchorsRes.statusCode, `${AREA.visibility} GET /multiplayer/systems`).toBe(200);
    const sectorAnchors = sectorAnchorsRes.json() as { systems: { systemId: string }[] };
    expect(
      sectorAnchors.systems.some((anchor) => anchor.systemId === foreignHomeSystem.id),
    ...
    expect(sectorPresenceRes.statusCode, `${AREA.visibility} GET /multiplayer/sectors/:sx/:sy/:sz/presence`).toBe(200);
    const sectorPresence = sectorPresenceRes.json() as { entities: { systemId: string }[] };
    expect(
      sectorPresence.entities.some((entity) => entity.systemId === foreignHomeSystem.id),
    ...
    expect(scoutRes.statusCode, `${AREA.discovery} scout through Jump Gate`).toBe(200);
    const scoutExpedition = (scoutRes.json() as { expedition: { id: string; result: { routeMode: string } } }).expedition;
    expect(scoutExpedition.result.routeMode, `${AREA.discovery} scout route mode`).toBe('jump_gate');
    await db
      .update(expeditions)
    ...
    expect(meAfterScout.statusCode, `${AREA.discovery} online sync after scout`).toBe(200);
    expect(
    ...
    expect(colonizerRes.statusCode, `${AREA.colonization} colonizer through Jump Gate`).toBe(200);
    const colonizerExpedition = (colonizerRes.json() as { expedition: { id: string; result: { routeMode: string } } }).expedition;
    expect(colonizerExpedition.result.routeMode, `${AREA.colonization} colonizer route mode`).toBe('jump_gate');
    await db
      .update(expeditions)
      .set({ eta: new Date(Date.now() - 1_000) })
    ...
    expect(meAfterColonizer.statusCode, `${AREA.colonization} online sync after colonizer`).toBe(200);
    expect(
    ...
    const commandCenters = await db.query.buildings.findMany({
      where: and(eq(buildings.planetId, targetPlanet.id), eq(buildings.typeId, 'command_center')),
    });
    expect(commandCenters, `${AREA.colonization} command center bootstrap is single`).toHaveLength(1);
    ...
    expect(cargoRes.statusCode, `${AREA.cargo} cargo transfer through Jump Gate`).toBe(200);
    const cargoExpedition = (cargoRes.json() as { expedition: { id: string; result: { routeMode: string } } }).expedition;
    expect(cargoExpedition.result.routeMode, `${AREA.cargo} cargo route mode`).toBe('jump_gate');
    await db
      .update(expeditions)
      .set({ eta: new Date(Date.now() - 1_000) })
    ...
    expect(meAfterCargo.statusCode, `${AREA.cargo} online sync after cargo`).toBe(200);
    expect(
    ...
    expect(storedCargo?.status, `${AREA.cargo} cargo expedition completed`).toBe('completed');
  });
});
