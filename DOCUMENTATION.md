...
`shared/config/` holds deterministic catalogs duplicated only when both backend and browser need identical numbers — today **`researchCatalog.ts`** (full tech tree + scaling notes, including Energy effects), **`buildingResearchGates.ts`** (advanced energy-building unlocks included), **`productionRecipes.ts`** (manual manufacturing and battery-charge recipes), and **`tutorialRewards.ts`** (tutorial iron/water bundles + EN/RU UI summaries).
