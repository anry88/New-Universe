...
- `components/UpgradeDialog.tsx` & `components/BuildDialog.tsx` — dialogs for managing buildings; the upgrade dialog shows and switches the saved extractor target, links production-capable buildings into `ProductionDialog`, while the build dialog groups options into localized category sections (energy, extraction, processing, logistics, shipbuilding, progress, special) and exposes extractor resource-choice chips with used/source counts.
...
`shared/config/` holds deterministic catalogs duplicated only when both backend and browser need identical numbers — today **`researchCatalog.ts`** (full tech tree + scaling notes, including Energy effects and realistic one-slot queue timers), **`buildingResearchGates.ts`** (advanced energy-building unlocks included), **`productionRecipes.ts`** (manual manufacturing and battery-charge recipes), and **`tutorialRewards.ts`** (tutorial iron/water bundles + EN/RU UI summaries).
