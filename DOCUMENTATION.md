...
- `components/ProductionDialog.tsx` — compact recipe picker and production-order confirmation sheet. It shows what will be produced, required inputs, output quantity, duration, storage/resource blocks, and active-order remaining-time countdowns before starting a job that spends inputs immediately.
