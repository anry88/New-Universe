...
- `components/ExpeditionDialog.tsx` — launch dialog for scouts and colonizers. Logistics ships route into `CargoTransferDialog`; local colonizer routes require a discovered unsettled home-system planet target and use shared same-system planet distance, while Jump Gate routes select known public destinations from `/jump-gate/state`, show safe unknown/discovered body choices, include Jump Fuel tank blocking, and use the same shared preview math as backend launch results.
