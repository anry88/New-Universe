...
- `components/CargoTransferDialog.tsx` — interplanetary logistics interface for moving resources between colonies through the shared multi-load cargo request contract; filters selectable ships to logistics-role transports, lets inter-system targets opt into a Jump Gate route with the fixed stored Jump Fuel cost, and localizes known cargo API errors.
