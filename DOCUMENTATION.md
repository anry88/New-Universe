...
- `jump_gates` — one private outer-orbit Home System Jump Gate per unlocked player/home system, with calibration target/timer state and random-jump cooldown timestamps. Unlock is derived server-side from completed `jump_drive >= 1`, so active research timers do not expose the gate.
...
- `jump-gate.ts` — Jump Gate state DTOs for `GET /jump-gate/state`, including locked reasons, the private home anchor, calibration state, random jump availability, and discovered public destination summaries.
