...
- Treat workstation env as unsafe input for `deploy-local.sh`: local `DATABASE_URL`, `REDIS_URL`, `VITE_API_URL`, `PUBLIC_FRONTEND_URL`, and similar variables may belong to local Docker image builds. Use a clean `env -i` invocation plus an explicit deploy env file instead of sourcing deploy secrets into the current shell.
