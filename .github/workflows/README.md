...
- **`deploy.yml`** — Disabled cloud deploy blocker for task **P4-DEP-002**. It still appears in Actions so accidental manual dispatches fail early with an explicit message, but it does not check out code, read GitHub Environment secrets, install `flyctl`, call `wrangler`, run migrations, or touch Fly.io / Cloudflare Pages. Current deploys run only from the operator machine with `scripts/deploy-hdc.sh --environment prod`. See [`docs/production/release-workflow.md`](../../docs/production/release-workflow.md).
