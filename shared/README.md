...
- `config/` — progression catalogs consumed by both Node and Vite bundles where duplication would drift (research gates for buildings; full tech tree).
    ...
    - **`resourceExtractionRates.ts`** — `EXTRACTABLE_RESOURCE_RATES_PER_HOUR` and `extractionRateForResource`; canonical per-resource passive extraction rates so iron, silicon, aluminum, titanium, oil, biomass, etc. do not all inherit the same building-level rate.
...
- `types/` — TypeScript interfaces and Zod schemas for HTTP payloads, WebSocket events, and other cross-cutting structures.
    ...
    - **`cargo.ts`** — `CargoTransferLoad` and `CargoTransferRequest` contracts for one-way cargo transfer orders; duplicate resource load lines are valid and aggregated by the backend.
