...
- `config/` — progression catalogs consumed by both Node and Vite bundles where duplication would drift (research gates for buildings; full tech tree).
    ...
    - **`researchCatalog.ts`** — canonical **7×5** research tree (`RESEARCH_CATALOG`, `RESEARCH_TECH_TREE`) including the Energy branch and typed energy generation/storage/efficiency effects; import via `@shared/config/researchCatalog` from backend and frontend (Docker mounts `shared/` at `/app/shared`; backend code uses `@shared`, not `../../../shared`).
...
- `types/` — TypeScript interfaces and Zod schemas for HTTP payloads, WebSocket events, and other cross-cutting structures.
    ...
    - **`research.ts`** — `ResearchBranch`, `ResearchProgress` (including active-tier `startedAt`), `ResearchDefinition`, `RushResearchRequest` / `RushResearchResponse`, `ResearchRequirementRef`, localized `RESEARCH_BRANCH_LABELS` / `researchBranchLabel`, `RESEARCH_BRANCH_LABELS_EN`, and `ResourceId` union (includes gameplay resources such as `oil`, `fuel`, `steel`, `electronics`, and tiered minerals) used by the tech tree and unlock messaging on backend/frontend.
