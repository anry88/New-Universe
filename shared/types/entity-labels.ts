...
export const SHIP_ENTITY_LABELS: Record<string, EntityLabels> = {
  ...
  medium_shield_ship: {
    en: "Medium Shield Ship",
    ru: "Средний щитовой корабль",
  },
  ...
  heavy_rocket_carrier: {
    en: "Heavy Rocket Carrier",
    ru: "Тяжёлый ракетный носитель",
  },
...
export const SHIP_ENTITY_DESCRIPTIONS: Record<string, EntityLabels> = {
  ...
  refueler: {
    en: "Support tanker with separate ordinary and Jump Fuel reserves for idle ships.",
    ru: "Корабль поддержки с отдельным резервом обычного и прыжкового топлива для свободных кораблей.",
  },
  light_fighter: {
...
export function shipDescription(
  typeId: string | null | undefined,
  locale: Locale = "en",
): string {
...
export function formatEntityList(
  ...
  const labels = [...new Set(entityIds ?? [])].map((entityId) =>
    labeler(entityId, locale),
  );
