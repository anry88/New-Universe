...
export const ANALYTICS_EVENT_DEFINITIONS = {
  ...
  ship_refuel_transfer_started: {
    category: "ships",
    surfaces: ["backend"],
    description:
      "A refueler was dispatched to transfer ordinary fuel or Jump Fuel to another ship.",
    safeProperties: ["fuel", "jumpFuel"],
  },
  ship_refuel_replenish_started: {
    category: "ships",
    surfaces: ["backend"],
    description:
      "A refueler was dispatched to an owned planet to refill tanks and support reserves.",
    safeProperties: ["targetPlanetId"],
  },
  research_started: {
    category: "research",
    surfaces: ["backend"],
  ...
  resource_conversion_completed: {
    ...
    surfaces: ["backend"],
    description:
      "A player converted an owned planet resource through a processor.",
  ...
  market_order_created: {
    ...
    surfaces: ["backend"],
    description:
      "A future player-market order was created after market settlement exists.",
  ...
  market_order_filled: {
    ...
    surfaces: ["backend"],
    description:
      "A future player-market order was settled after escrowed market settlement exists.",
    safeProperties: ["resourceId", "orderType", "amountBand", "priceBand"],
  },
  market_order_cancelled: {
    ...
    surfaces: ["backend"],
    description:
      "A future player-market order was cancelled and escrow returned.",
  ...
  system_renamed: {
    ...
    surfaces: ["backend"],
    description:
      "A system was renamed by a player holding the only colonies in it.",
    safeProperties: [
      "renameCount",
      "diamondsSpent",
      "diamondsRemaining",
...
export function isKnownAnalyticsEventName(
  eventName: string,
...
export function sanitizeAnalyticsEventProperties(
  ...
  const allowedProperties = new Set<string>(
    ANALYTICS_EVENT_DEFINITIONS[eventName].safeProperties,
  );
  const safe: SafeAnalyticsProperties = {};

  for (const [key, value] of Object.entries(properties)) {
    if (Object.keys(safe).length >= MAX_ANALYTICS_PROPERTIES) break;
    if (!allowedProperties.has(key) || !isSafeAnalyticsPropertyKey(key))
      continue;
